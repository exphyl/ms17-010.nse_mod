...
description = [[
	...
	The script connects to the $IPC tree, executes a transaction on FID 0 and checks if the error 
	"STATUS_INSUFF_SERVER_RESOURCES" is returned to determine if the target is not patched against ms17-010.

	Tested on a vulnerable Windows 7. We might have some issues with v2 protocols with signing enabled.
...
local function check_ms17010(host, port, sharename)
	...
	if not status then
		...
		t = {
			">I2 I2 I2 I2 B B I2 I4 I2 I2 I2 I2 I2 B B I2 I2 I2 I2 I2 I2",
		...
		-- smb_params = string.pack(">I2 I2 I2 I2 B B I2 I4 I2 I2 I2 I2 I2 B B I2 I2 I2 I2 I2 I2",
		  ...
		  -- 0x455c   --E\
		-- )
		stdnse.print_debug("SMB: Sending SMB_COM_TRANSACTION")
...
--		print("smb_header")
--		print(smb_header)
--		print(string.gsub(smb_header, "ERROR:", ",ERROR:"))

		scT = split(string.gsub(smb_header, "ERROR:", ",ERROR:"), ",")
		smb_cmd = split(scT[1], ": ")[2]
		err = split(scT[2], ": ")[2]
		
--		print ("smb_cmd".." = "..smb_cmd)
--		print ("err".." = "..err)
--		print("-------------------------")
...
action = function(host,port)
	...
	local vuln = {
		...
		IDS = {CVE = 'CVE-2017-0143'},
		risk_factor = "HIGH",
		description = "A critical remote code execution vulnerability exists in Microsoft SMBv1 servers (ms17-010).",
		references = {
		'https://technet.microsoft.com/en-us/library/security/ms17-010.aspx',
		'https://blogs.technet.microsoft.com/msrc/2017/05/12/customer-guidance-for-wannacrypt-attacks/'
		},
